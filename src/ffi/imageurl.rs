...
fn raise_args_exception() {
    unsafe {
        let msg = "orjson.ImageUrl() takes exactly 1 positional argument";
        let err_msg =
...
#[cold]
#[cfg_attr(feature = "optimize", optimize(size))]
pub unsafe extern "C" fn orjson_imageurl_tp_new(
    ...
    unsafe {
        if Py_SIZE(args) != 1 || !kwds.is_null() {
            ...
            let url = PyTuple_GET_ITEM(args, 0);
            // Validate that the argument is a string
            if (*url).ob_type != crate::typeref::STR_TYPE {
                let msg = "orjson.ImageUrl() argument must be a string";
                let err_msg =
                    PyUnicode_FromStringAndSize(msg.as_ptr() as *const c_char, msg.len() as isize);
                PyErr_SetObject(PyExc_TypeError, err_msg);
                Py_DECREF(err_msg);
            ...
            let obj = Box::new(ImageUrl {
                ...
                #[cfg(not(Py_GIL_DISABLED))]
                ob_refcnt: 1,
                ob_type: crate::typeref::IMAGEURL_TYPE,
                url: url,
            });
...
#[cold]
#[cfg_attr(feature = "optimize", optimize(size))]
pub unsafe extern "C" fn orjson_imageurl_dealloc(object: *mut PyObject) {
    unsafe {
        Py_DECREF((*(object as *mut ImageUrl)).url);
        std::alloc::dealloc(object as *mut u8, std::alloc::Layout::new::<ImageUrl>());
    }
}

#[cfg(Py_GIL_DISABLED)]
const IMAGEURL_TP_FLAGS: AtomicU64 = AtomicU64::new(Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE);

#[cfg(all(Py_3_10, not(Py_GIL_DISABLED)))]
const IMAGEURL_TP_FLAGS: core::ffi::c_ulong = Py_TPFLAGS_DEFAULT | Py_TPFLAGS_IMMUTABLETYPE;

#[cfg(not(Py_3_10))]
const IMAGEURL_TP_FLAGS: core::ffi::c_ulong = Py_TPFLAGS_DEFAULT;

#[unsafe(no_mangle)]
#[cold]
#[cfg_attr(feature = "optimize", optimize(size))]
pub unsafe extern "C" fn orjson_imageurltype_new() -> *mut PyTypeObject {
    unsafe {
        let ob = Box::new(PyTypeObject {
            ...
            tp_name: "orjson.ImageUrl\0".as_ptr() as *const c_char,
            tp_basicsize: core::mem::size_of::<ImageUrl>() as isize,
            tp_itemsize: 0,
            tp_dealloc: Some(orjson_imageurl_dealloc),
            tp_init: None,
            tp_new: Some(orjson_imageurl_tp_new),
            tp_flags: IMAGEURL_TP_FLAGS,
            // ...
            tp_bases: null_mut(),
            tp_cache: null_mut(),
            tp_del: None,
